...
def get_weak_concepts(
    ...
    *,
    ...
    rows = (
        ...
        .order_by(
            func.count(models.WrongAnswer.id).desc(),
            models.WrongAnswer.concept.asc(),
        )
        .all()
    )
    ...
    return [
        {
            ...
            "recommendation": (
                f"{concept or '미분류'} 개념을 우선 복습하세요."
            ),
        }
        for concept, wrong_count in rows
    ]
    ...
    *,
    ...
    if exam_date is None:
        raise InvalidRequestError (
            "exam_date를 YYYY-MM-DD 형식으로 입력하세요."
        )
    ...
    try:
        ...
        raise InvalidRequestError (
            "exam_date 형식이 올바르지 않습니다. 예: 2026-08-01"
        )
    ...
    if days_left == 0:
        ...
        plan.append(
            {
                "day": "D-1",
                "task": "핵심 개념과 오답 노트를 중심으로 최종 정리하세요.",
                "concepts": [item["concept"] for item in weak_concepts[:5]],
            }
        )
