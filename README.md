...
- 학습 자료, 오답, 문제·RAG 평가, 응시 기록, 학습 목표, 체크리스트, 학습 세션, 스마트 복습 큐를 `users.id` 외래키로 연결
- 사용자 소유 테이블의 중복 `user_name` 문자열을 제거하고 내부 `user_id`를 단일 소유권 기준으로 사용
...
- `user_id` 역시 공개 API 입력 필드가 아닙니다. 서버가 인증된 사용자의 `users.id`를 내부적으로 주입합니다.
...
- v0.4.4 마이그레이션은 기존 사용자 소유 데이터의 `user_name`과 `users.user_name`을 일치시켜 `user_id`를 백필합니다. 기존 데이터에 대응하는 사용자 계정이 없으면 최종 마이그레이션이 중단됩니다.
- 최종 마이그레이션은 사용자 소유 테이블에 `user_id NOT NULL`을 적용하고 기존 `user_name` 열을 제거하므로, 적용 전에 데이터베이스를 백업하고 미매칭 사용자 데이터를 정리해야 합니다.
- `e1d39763c3a4_finalize_user_id_ownership` 마이그레이션의 downgrade는 구현되어 있지 않아 자동 롤백을 지원하지 않습니다.
- 외래키에 연쇄 삭제(`ON DELETE CASCADE`)가 설정되어 있지 않습니다. 관련 데이터가 남아 있는 사용자나 학습 자료 삭제 기능을 추가하려면 삭제 정책을 별도로 설계해야 합니다.
- PostgreSQL 사용자 소유권은 외래키로 정규화되지만 Chroma 문서 metadata에는 검색과 필터링을 위해 `user_name`이 계속 저장됩니다.
